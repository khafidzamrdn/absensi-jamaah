/* ============================================================
   ABSENSI JAMAAH - script.js v1.0.1
   (Versi robust: login selalu responsif walau IndexedDB gagal)
   ============================================================ */
...
function openDB() {
  return new Promise((resolve, reject) => {
    try {
      ...
      req.onsuccess = () => { db = req.result; dbAvailable = true; resolve(); };
      req.onerror = () => reject(req.error);
      req.onblocked = () => reject(new Error('IndexedDB diblokir'));
    } catch (err) {
      reject(err);
    }
  });
...
/* ================== PIN / SESSION ================== */
function getPin() {
  try { return localStorage.getItem('absensi_pin') || DEFAULT_PIN; }
  catch (e) { return DEFAULT_PIN; }
}
function setPin(v) {
  try { localStorage.setItem('absensi_pin', v); } catch (e) {}
}
function isLoggedIn() {
  try { return localStorage.getItem('absensi_logged_in') === '1'; }
  catch (e) { return false; }
}
function setLoggedIn(v) {
  try {
    if (v) localStorage.setItem('absensi_logged_in', '1');
    else localStorage.removeItem('absensi_logged_in');
  } catch (e) {}
}
...
function toggleTheme() {
  ...
  try { localStorage.setItem('absensi_theme', state.theme); } catch (e) {}
  applyTheme();
}
  ...
  wrapWaktu.innerHTML = PRAYERS.map(w => {
    ...
    return `
      <div class="card">
        ...
        <div class="stat-line"><span>🟡 Telat</span><b>${pt}% <span class="muted small">(${d[
